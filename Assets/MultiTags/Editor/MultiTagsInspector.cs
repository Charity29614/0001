...
public class ObjectBuilderEditor : Editor
{
	...
	public string stringToEdit = "//MultiTags are NOT case sensitive! :)\n\n" +
        ...
        // Prevent "Untagged" from being added to localTagList
        foreach (var item in myScript.localTagList.ToArray()) // Using ToArray() to avoid modification during iteration
        {
            if (item.Name == "Untagged") // Ensure no "Untagged" tags are added
            {
                myScript.localTagList.Remove(item); // Remove the "Untagged" tag
