...
namespace DigitalRuby.RainMaker
{
    public class BaseRainScript : MonoBehaviour
    {
        ...
        [Range(0f, 1f)]
        [Tooltip("Adjusts the volume of rain sounds")]
        [SerializeField] private float RainVolume = 1f;

        // Property to get/set the RainVolume value and call UpdateRainVolume when it changes
        public float RainVolumeValue
        {
            get
            {
                return RainVolume;
            ...
            set
            {
                RainVolume = Mathf.Clamp(value, 0f, 1f); // Ensure it's always within the range
                OnValueChanged(RainVolume); // Call ValueChanged to handle logic when volume changes
            }
        }

        // Method that gets called when the value changes (can be used for more logic)
        public void OnValueChanged(float newValue)//ahhaha
        {
            if (RainSoundAudioMixer != null)
            {
                // If the volume is 0, set the volume to the lowest possible value in the Audio Mixer
                if (Mathf.Approximately(newValue, 0f))
                {
                    RainSoundAudioMixer.audioMixer.SetFloat("RainVolume", -80f); // 0 volume for AudioMixer
                }
                else
                {
                    RainSoundAudioMixer.audioMixer.SetFloat("RainVolume", Mathf.Log10(newValue) * 20);
                    //Debug.Log($"Changed to: {Mathf.Log10(newValue) * 20}");
                }
            }
        }
        ...
        private void OnValidate()
        {
            if (RainSoundAudioMixer != null)
            {
                ...
                RainSoundAudioMixer.audioMixer.GetFloat("RainVolume", out oldVolume);

                if (!Mathf.Approximately(oldVolume, Mathf.Log10(RainVolumeValue) * 20))
                {
                    OnValueChanged(RainVolumeValue);
                }
            }
        }
        ...
        //{
        //    if (audioSourceRainLight?.AudioSource != null)
        //        audioSourceRainLight.AudioSource.volume = RainVolume;

        //    if (audioSourceRainMedium?.AudioSource != null)
        //        audioSourceRainMedium.AudioSource.volume = RainVolume;

        //    if (audioSourceRainHeavy?.AudioSource != null)
        //        audioSourceRainHeavy.AudioSource.volume = RainVolume;

        //    if (audioSourceWind?.AudioSource != null)
        //        audioSourceWind.AudioSource.volume = RainVolume * WindSoundVolumeModifier;
        //}
        ...
        {
            //RainVolumeSlider = GetComponent<Slider>();
            //RainVolumeSlider.onValueChanged.AddListener((value) =>
            //{

            //});
            OnValueChanged(RainVolume);
#if DEBUG
        ...
        protected virtual void Update()
        {
            ...
            //// Check if RainVolume has changed
            //if (Math.Abs(previousRainVolume - RainVolume) > 0.01f) // Avoid minor floating-point inaccuracies
            //{
            //    previousRainVolume = RainVolume;
            //    UpdateRainVolume(); // Update the rain volume dynamically
            //}
        }
